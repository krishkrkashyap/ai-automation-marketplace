...
INDUSTRY_DATA = {
    'ecommerce-automation': {'name': 'E-Commerce Automation', 'icon': 'E-Commerce', 'description': 'Transform your online store with AI-powered automation.', 'problems': ['Manual inventory', 'Slow response', 'Order processing'], 'solutions': ['AI prediction', 'Chatbots', 'Automation']},
    'finance-automation': {'name': 'Finance Automation', 'icon': 'Finance', 'description': 'Revolutionize financial operations.', 'problems': ['Manual invoice', 'Reconciliation', 'Slow reporting'], 'solutions': ['OCR', 'Automation', 'Dashboards']},
    'healthcare-automation': {'name': 'Healthcare Automation', 'icon': 'Healthcare', 'description': 'Streamline healthcare operations.', 'problems': ['Admin overhead', 'Scheduling', 'Transcription'], 'solutions': ['Smart scheduling', 'AI transcription', 'EMR']},
}

BOTS_DATA = {
    'lead-qualification-bot': {'name': 'Lead Qualification Bot', 'industry': 'Sales', 'description': 'Automatically qualify leads 24/7.', 'features': ['Lead scoring', 'CRM integration', 'Notifications'], 'use_cases': ['E-Commerce', 'SaaS']},
    'invoice-processing-bot': {'name': 'Invoice Processing Bot', 'industry': 'Finance', 'description': 'Automate invoice extraction.', 'features': ['OCR', 'Validation', 'Workflow'], 'use_cases': ['Finance']},
    'customer-support-bot': {'name': 'Customer Support Bot', 'industry': 'Service', 'description': 'AI-powered 24/7 support.', 'features': ['NLP', 'Multi-language', 'Knowledge base'], 'use_cases': ['E-Commerce']},
    'fraud-detection-bot': {'name': 'Fraud Detection Bot', 'industry': 'Security', 'description': 'Real-time fraud detection.', 'features': ['Monitoring', 'Pattern recognition', 'Risk scoring'], 'use_cases': ['Banking']},
}
...
def industry_detail(request, industry_slug):
    industry = INDUSTRY_DATA.get(industry_slug, {'name': industry_slug.replace('-', ' ').title(), 'icon': 'AI', 'description': 'AI automation', 'problems': ['Manual'], 'solutions': ['Automation']})
...
def bot_detail(request, bot_slug):
    bot = BOTS_DATA.get(bot_slug, {'name': bot_slug.replace('-', ' ').title(), 'industry': 'General', 'description': 'AI bot', 'features': ['Feature'], 'use_cases': ['All']})
    return render(request, 'pages/bot_detail.html', {'bot': bot, 'slug': bot_slug})

def case_studies(request):
    from pages.models import CaseStudy
    cases = CaseStudy.objects.filter(is_published=True)[:6]
    return render(request, 'pages/case_studies.html', {'cases': cases})
...
def pricing(request):
    pricing_tiers = [
        {'name': 'Starter', 'price': '499', 'description': 'Small businesses', 'features': ['1 Bot', 'Basic support']},
        {'name': 'Professional', 'price': '1499', 'description': 'Growing businesses', 'features': ['3 Bots', 'Priority support']},
        {'name': 'Enterprise', 'price': 'Custom', 'description': 'Large organizations', 'features': ['Unlimited', '24/7 support']},
    ]
